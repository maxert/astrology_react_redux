...
function DropdownExampleInline({ ID, Type, Favorite,isFavortie }) {  
  ...
  const { none, Add_favorite } = useContext(
    ReduceContext
  );
  const { display,search_delete,search_add_favorite,search_delete_favorite } = useContext(ShowContext);
  return (
    <Dropdown className={"edit_drop"} inline floating labeled button>
      <Dropdown.Menu>
        {isFavortie ===true ? (
          <Dropdown.Item
            data-index="0"
            onClick={() => search_delete_favorite(ID, Type,display.data_value,Favorite)}
          >
          ...
          <Dropdown.Item
            data-index="0"
            onClick={() => search_add_favorite(ID, Type,display.data_value,Favorite)}
          >
        ...
        <Dropdown.Item
          data-index="2"
          onClick={() => {search_delete(ID,none.data_link_favorite.type_link,display.data_value)}}
        >
