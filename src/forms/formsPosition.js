...
import { Input, Button, Checkbox, Form, Ref } from "semantic-ui-react";
  ...
  return (
    <form className="planetary_position" onSubmit={handleSubmit(onSubmit)}>
      ...
      <Checkbox
        ...
        onClick={() => (isPlay ? (setPlay(false),setRefresh(1)) : (setPlay(true),setRefresh(0)))}
      />
      <div className="header_card">
        <div className="select_submit">
          <button className="prev_button" onClick={()=>setint_d(-1)}></button>
          <Input className="numbers" name="text" type="number" defaultValue={interval} onChange={(e,data)=>data.value<0?"":setNumber(data.value)}  />
          <SelectWeeks SelectSubmite={(event,data)=>onChangeSelect(event,data)}></SelectWeeks>
        ...
        <Button className="button_reset" onClick={()=>setRefresh(1)}>Сбросить</Button>
      </div>
